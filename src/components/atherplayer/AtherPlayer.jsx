            ...
            >
              ...
              <div className="notes-list mb-4">
                ...
                {data.timeline?.map((item, i) => (
                  <div key={`tl-${i}`} className="mb-4" style={{ backgroundColor: "#1e2130", borderRadius: "12px", padding: "20px", border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div className="mb-3 text-uppercase" style={{ fontSize: "0.75rem", fontWeight: "800", color: "#f8f9fa", letterSpacing: "1px" }}>
                    ...
                    {item.type === "code" ? (
                      <div className="position-relative">
                        <button 
                          ...
                          style={{ cursor: "pointer", color: "#a5b4fc", backgroundColor: "rgba(165,180,252,0.1)" }}
                          title="Copy code"
                        >
                          <i className="bi bi-clipboard me-1"></i> Copy
                        </button>
                        <pre className="p-4 bg-black rounded text-light overflow-auto" style={{ fontSize: "0.9rem", borderBottom: "10px solid #3b4261", fontFamily: "monospace", margin: 0, whiteSpace: "pre" }}>
                          {item.text}
                        </pre>
                      </div>
                    ) : item.type === "quote" ? (
                      <blockquote className="border-start border-4 border-accent ps-3 text-light fst-italic my-2" dangerouslySetInnerHTML={{ __html: item.text }}></blockquote>
                    ) : (
                      <div className="text-light opacity-75" style={{ fontSize: "0.95rem", lineHeight: "1.6" }} dangerouslySetInnerHTML={{ __html: item.text }}></div>
                    )}
                  </div>
                ))}
                {data.suggestions && data.suggestions.length > 0 && (
                  <div className="mt-4 mb-4 d-flex flex-wrap gap-2">
                    {data.suggestions.map((sug, i) => (
                      <button key={`init-sug-${i}`} onClick={() => handleAsk(sug)} className="btn btn-sm btn-outline-light rounded-pill border-opacity-25" style={{ fontSize: "0.8rem", color: "#a5b4fc", borderColor: "rgba(165,180,252,0.5)", backgroundColor: "rgba(165,180,252,0.05)" }}>
                        <i className="bi bi-box-arrow-up-right me-2"></i> {sug}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="chat-history mt-5 pt-4">
                ...
                {chatHistory.slice(2).map((msg, i) => {
                  if (msg.role === "user") {
                    return (
                      <div key={`chat-${i}`} className="mb-4 ms-5 p-3 rounded" style={{ backgroundColor: "rgba(255,255,255,0.05)" }}>
                         <div className="small fw-bold mb-2 text-white">You</div>
                         <div className="text-light" style={{ fontSize: "0.95rem", whiteSpace: "pre-wrap" }}>{msg.parts[0].text}</div>
                      </div>
                    );
                  } else {
                    ...
                    if (suggestionMatch) {
                      try {
                        ...
                        if (Array.isArray(parsedArr)) chatSuggestions = parsedArr;
                        rawContent = rawContent.replace(suggestionMatch[0], "").trim();
                      } catch(e) {}
                    }
                    ...
                    return (
                      <div key={`chat-${i}`} className="mb-4 me-2">
                        {parts.map((part, pIdx) => {
                          ...
                          const isCode = part.trim().startsWith("```") && part.trim().endsWith("```");
                          ...
                          if (isCode) {
                            ...
                            rawText = lines.slice(1, -1).join('\n'); // remove ``` tags
                             ...
                             // basic bold Markdown parsing since dangerouslySetInnerHTML is used
                             rawText = rawText.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
                          }

                          return (
                            <div key={`block-${pIdx}`} className="mb-3" style={{ backgroundColor: "#1e2130", borderRadius: "12px", padding: "20px", border: "1px solid rgba(255,255,255,0.05)" }}>
                              <div className="mb-3 text-uppercase" style={{ fontSize: "0.75rem", fontWeight: "800", color: "#f8f9fa", letterSpacing: "1px" }}>
                                {isCode ? "CODE" : "SUMMARY"}
                              </div>
                              {isCode ? (
                                ...
                                <div className="text-light opacity-75" style={{ fontSize: "0.95rem", lineHeight: "1.6" }} dangerouslySetInnerHTML={{ __html: rawText }}></div>
                              )}
                            </div>
                          );
                        })}
                        {chatSuggestions.length > 0 && (
                          <div className="mt-3 d-flex flex-wrap gap-2">
                            {chatSuggestions.map((sug, sIdx) => (
                              <button key={`chatsug-${i}-${sIdx}`} onClick={() => handleAsk(sug)} className="btn btn-sm btn-outline-light rounded-pill border-opacity-25" style={{ fontSize: "0.8rem", color: "#a5b4fc", borderColor: "rgba(165,180,252,0.5)", backgroundColor: "rgba(165,180,252,0.05)" }}>
                                <i className="bi bi-box-arrow-up-right me-1"></i> {sug}
                              </button>
                            ))}
                          </div>
                        )}
